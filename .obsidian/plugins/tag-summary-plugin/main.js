...
var SummarySettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian.Setting(containerEl).setName("显示 Callouts （Fendi 2024-09-23 汉化）").setDesc("显示 callout 块内的文本").addToggle((toggle) => toggle.setValue(this.plugin.settings.includecallout).onChange((value) => __async(this, null, function* () {
      this.plugin.settings.includecallout = value;
      yield this.plugin.saveSettings();
    })));
    new import_obsidian.Setting(containerEl).setName("显示链接").setDesc("显示指向原始笔记的链接").addToggle((toggle) => toggle.setValue(this.plugin.settings.includelink).onChange((value) => __async(this, null, function* () {
      this.plugin.settings.includelink = value;
      yield this.plugin.saveSettings();
    })));
    new import_obsidian.Setting(containerEl).setName("移除标签").setDesc("从文本中删除标签").addToggle((toggle) => toggle.setValue(this.plugin.settings.removetags).onChange((value) => __async(this, null, function* () {
      this.plugin.settings.removetags = value;
      yield this.plugin.saveSettings();
    })));
    new import_obsidian.Setting(containerEl).setName("列表项目").setDesc("仅包括包含标签的列表中的项，而不是整个列表").addToggle((toggle) => toggle.setValue(this.plugin.settings.listparagraph).onChange((value) => __async(this, null, function* () {
      this.plugin.settings.listparagraph = value;
      yield this.plugin.saveSettings();
    })));
    new import_obsidian.Setting(containerEl).setName("包含子项目").setDesc("包括列表中的子项.").addToggle((toggle) => toggle.setValue(this.plugin.settings.includechildren).onChange((value) => __async(this, null, function* () {
...
var SummaryModal = class extends import_obsidian2.Modal {
  ...
  onOpen() {
    ...
    let { contentEl } = this;
    contentEl.createEl("h1", { text: "添加汇总" });
    if (listTags.length <= 0) {
      contentEl.setText("There are no tags in your notes");
    } else {
      this.include = listTags[0];
      new import_obsidian2.Setting(contentEl).setName("选择要包括在摘要中的标签").addDropdown((menu) => {
      ...
      new import_obsidian2.Setting(contentEl).setName("选择要排除在摘要中的标签").addDropdown((menu) => {
      ...
      new import_obsidian2.Setting(contentEl).addButton((button) => {
        button.setButtonText("添加汇总").setCta().onClick(() => {
          this.close();
